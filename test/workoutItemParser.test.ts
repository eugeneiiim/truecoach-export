...
test('opener', (t) => {
  ...
  const sets = workoutItemParser.infoToSets(date, exercise, '• Opener\n')

  t.true(_.isEqual(sets, [{ date, exercise, reps: '1', rpe: 'Opener' }]))
})
...
test('handles rep range', (t) => {
  ...
  const sets = workoutItemParser.infoToSets(date, exercise, '15-20 @8 x 3 sets')
